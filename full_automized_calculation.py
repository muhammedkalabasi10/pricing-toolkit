...
def main():
    ...
    # Find "Güncel Fiyat" column (support both 'güncel' and 'guncel')
    guncel_col = find_col(ws, lambda s: ("güncel" in s.lower() or "guncel" in s.lower()) and "fiyat" in s.lower())
    ...
    try:
        prompt = f"Teklif bazında Kâr değeri girin (örn 50 veya 50,00). Boş bırakmak için Enter: "
        user_raw = input(prompt).strip()
    except (EOFError, KeyboardInterrupt):
        # If input is interrupted, treat as empty input
        user_raw = ""

    # Parse user's desired profit (kâr) once; supports Turkish number formats
    ...
    for r in range(2, ws.max_row + 1):
        ...
        # --- Interactive price prompt & conditional writing ---
        # Ask user for a proposed price for this SKU. If user input is empty or invalid,
        # we fall back to writing the current price (if available) in red.
        

        # Compare user's desired profit (kâr) with profit_vat
        if user_profit is not None and profit_vat is not None and user_profit > profit_vat:
            # User's requested profit is higher than calculated profit_vat -> write truncated max price
            raw_max = ws.cell(r, max_col).value
            max_val = parse_turkish_number(raw_max)
            if max_val is not None:
                truncated = int(max_val)  # drop fractional part
                ws.cell(r, fiyatgir_col).value = truncated
            else:
                # If max value can't be parsed, write raw content
                ws.cell(r, fiyatgir_col).value = raw_max
            ws.cell(r, fiyatgir_col).fill = green_fill
        else:
            # Use current price (if any) and mark red

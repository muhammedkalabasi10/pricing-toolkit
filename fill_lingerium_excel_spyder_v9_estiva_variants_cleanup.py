...
def build_default_output_path(template_path: str) -> str:
    template_file = Path(template_path).resolve()
    return str(template_file.with_name(f"{template_file.stem}_completed{template_file.suffix}"))
...
def remove_invalid_estiva_rows(ws, headers: dict):
    barcode_col = headers.get("Barkod")
    color_col = headers.get("Renk")
    size_col = headers.get("Beden")
    if not barcode_col:
        return 0

    rows_to_delete = []
    for row_idx in range(2, ws.max_row + 1):
        barcode = normalize_barcode_key(ws.cell(row_idx, barcode_col).value)
        if not barcode.startswith("ESTIVA-"):
            continue

        color = clean_text(ws.cell(row_idx, color_col).value) if color_col else ""
        size = clean_text(ws.cell(row_idx, size_col).value) if size_col else ""

        # Önceki hatalı sürümlerde barkodlar ESTIVA-22801--38 veya
        # ESTIVA-22801--RENKSECINIZ gibi renksiz oluşuyordu. Bu satırlar
        # yeni doğru barkodlarla çakışmadığı için güncelleme değil silme gerekir.
        if "--" in barcode or not color or not size or is_estiva_placeholder_option(color) or is_estiva_placeholder_option(size):
            rows_to_delete.append(row_idx)
...
def is_estiva_url(page_url: str | None) -> bool:
    if not page_url:
        return False
    try:
        ...
        return False
    return "estiva.com.tr" in host or host == "estiva.com"


def is_estiva_html(html: str) -> bool:
    haystack = (html or "").lower()
    return "estiva.com.tr" in haystack and "product-detail-tab" in haystack


def detect_source_site(html: str, page_url: str | None = None) -> str:
    if is_estiva_url(page_url) or is_estiva_html(html):
        return "estiva"
    return "lingerium"


def text_after_label(soup: BeautifulSoup, label_text: str) -> str:
    for row in soup.select(".product-list-row"):
        title = row.select_one(".product-list-title")
        content = row.select_one(".product-list-content")
        if not title or not content:
            continue
        if normalize_tr(clean_text(title.get_text(" ", strip=True))).lower() == normalize_tr(label_text).lower():
            return clean_text(content.get_text(" ", strip=True))
    return ""


def normalize_token(value: str) -> str:
    value = normalize_tr(clean_text(value)).upper()
...
def split_product_name_and_color(product_name: str) -> tuple[str, str]:
    name = clean_text(product_name)
    if " - " in name:
        base_name, color = name.rsplit(" - ", 1)
        return clean_text(base_name), clean_text(color)
    return name, ""


def is_estiva_placeholder_option(value: str) -> bool:
    normalized = normalize_tr(clean_text(value)).lower()
    normalized = re.sub(r"[^a-z0-9]+", " ", normalized).strip()
    if not normalized:
        return True
    # Estiva dropdownlarında ilk seçenekler "Renk Seçiniz" ve "Beden Seçiniz" şeklinde
    # geliyor. Bunlar gerçek varyant olmadığı için Excel'e yazılmamalı.
    return "seciniz" in normalized or normalized in {"renk sec", "beden sec", "sec"}


def detect_estiva_variant_options(soup: BeautifulSoup, group_name: str) -> list[str]:
    target = normalize_tr(group_name).lower()
    values = []

    for variant_list in soup.select(".variant-list"):
        title_node = variant_list.select_one(".variant-group-title")
        title = clean_text(title_node.get_text(" ", strip=True)) if title_node else ""
        if normalize_tr(title).lower() != target:
            continue

        for option in variant_list.select('select[data-selector="select-dropdown-variant"] option'):
            value = clean_text(option.get_text(" ", strip=True))
            if is_estiva_placeholder_option(value):
                continue
...
def detect_estiva_sizes(soup: BeautifulSoup) -> list[str]:
    sizes = detect_estiva_variant_options(soup, "Beden")
    if sizes:
        return sizes

    # Eski/alternatif HTML yapıları için güvenli geri dönüş: sadece beden gibi görünen
    # seçenekleri al, renk veya placeholder seçeneklerini alma.
    fallback_sizes = []
    for option in soup.select('select[data-selector="select-dropdown-variant"] option'):
        value = clean_text(option.get_text(" ", strip=True))
        if is_estiva_placeholder_option(value):
            continue
        normalized = normalize_tr(value).upper().replace(" ", "")
        if re.fullmatch(r"[0-9]{1,3}|[XSML]{1,4}|[0-9]{1,3}[-/][0-9]{1,3}", normalized):
            if value not in fallback_sizes:
                fallback_sizes.append(value)
    return fallback_sizes


def detect_estiva_colors(soup: BeautifulSoup) -> list[str]:
    return detect_estiva_variant_options(soup, "Renk")


def detect_estiva_images(soup: BeautifulSoup) -> list[str]:
    images = []
    for tag in soup.select("div.thumb-item.image-cover a[data-image]"):
        candidate = normalize_image_url(tag.get("data-image") or "")
        if candidate and candidate not in images:
            images.append(candidate)

    if not images:
        for tag in soup.select("div.thumb-item.image-cover img[src]"):
            candidate = normalize_image_url(tag.get("src") or "")
            if candidate and candidate not in images:
                images.append(candidate)
...
def detect_estiva_description(soup: BeautifulSoup) -> str:
    detail = soup.select_one(".product-detail-tab .product-detail")
    if detail:
        return clean_text(detail.get_text(" ", strip=True))
    return ""


def detect_estiva_category(soup: BeautifulSoup) -> str:
    node = soup.select_one(".product-categories .product-list-content a")
    if node:
        return clean_text(node.get_text(" ", strip=True))
    return ""


def detect_estiva_price_info(soup: BeautifulSoup) -> tuple[float | None, float | None]:
    sale_price = detect_price(soup)
    market_price = None

    old_price_node = soup.select_one('.product-price-old')
    if old_price_node:
        market_price = parse_price(old_price_node.get_text(" ", strip=True))

    new_price_node = soup.select_one('.product-price-new')
    if new_price_node:
        discounted_price = parse_price(new_price_node.get_text(" ", strip=True))
        if discounted_price is not None and discounted_price > 0:
            sale_price = discounted_price

    if market_price is None and sale_price is not None:
        market_price = sale_price + 100

    return sale_price, market_price


def build_estiva_model_code(product_name: str, stock_code: str) -> str:
    base_name, _ = split_product_name_and_color(product_name)
    root_stock_code = clean_text(stock_code).split("-", 1)[0]
    brand_token = "ESTIVA"
    if root_stock_code:
        return f"{brand_token}-{normalize_token(root_stock_code)}"
    if base_name:
        return f"{brand_token}-{normalize_token(base_name)[:24]}"
    return brand_token


def parse_estiva_product(html: str, page_url: str | None = None) -> dict:
    soup = BeautifulSoup(html, "html.parser")

    product_name = clean_text((soup.select_one(".product-title h1") or soup.find("h1")).get_text(" ", strip=True))
    stock_code_on_page = text_after_label(soup, "Stok Kodu")
    price, market_price = detect_estiva_price_info(soup)
    sizes = detect_estiva_sizes(soup)
    base_name, color_from_name = split_product_name_and_color(product_name)
    dropdown_colors = detect_estiva_colors(soup)
    colors = [color_from_name] if color_from_name else dropdown_colors
    colors = [color for color in dict.fromkeys(colors) if color and not is_estiva_placeholder_option(color)]
    images = detect_estiva_images(soup)
    description = detect_estiva_description(soup)
    category_name = detect_estiva_category(soup)
    model_code = build_estiva_model_code(product_name, stock_code_on_page)

    variant_rows = []
    for color in colors or [""]:
        if is_estiva_placeholder_option(color):
            continue
        for size in sizes or [""]:
            if is_estiva_placeholder_option(size):
                continue
            variant_rows.append({
                "color": color,
                "size": size,
    ...
    product = {
        ...
        "product_color": colors[0] if colors else "",
        "product_code": stock_code_on_page,
        "price": price,
        "colors": colors,
        "sizes": sizes,
        ...
        "model_number": clean_text(stock_code_on_page).split("-", 1)[0],
        "model_code": model_code,
        "color_links": {color: page_url for color in colors} if page_url else {},
        "variant_rows": variant_rows,
        "variant_sections": {},
        "color_details": {
            color: {"sizes": sizes, "images": images, "url": page_url}
            for color in colors
        },
        "shipping_time": 2,
        "desi": 1,
...
def parse_product(html: str, page_url: str | None = None) -> dict:
    site = detect_source_site(html, page_url=page_url)
    if site == "estiva":
        return parse_estiva_product(html, page_url=page_url)
...
def index_existing_barcodes(ws, headers: dict) -> dict:
    ...
    for row_idx in range(2, ws.max_row + 1):
        ...
        barcode = normalize_barcode_key(value)
        if barcode:
...
def open_workbook_for_upsert(template_path: str, output_path: str):
    ...
    if removed_duplicate_rows:
        headers = {ws.cell(1, c).value: c for c in range(1, ws.max_column + 1)}
    removed_invalid_estiva_rows = remove_invalid_estiva_rows(ws, headers)
    if removed_invalid_estiva_rows:
        headers = {ws.cell(1, c).value: c for c in range(1, ws.max_column + 1)}
    ...
    return {
        ...
        "removed_invalid_estiva_rows": removed_invalid_estiva_rows,
    }



def set_cell_if_header_exists(values: dict, headers: dict, mapping: dict[str, object], *aliases: str):
    for alias in aliases:
        if alias in headers:
            values[alias] = mapping
            return True
    return False
...
def upsert_product_to_sheet(session: dict, product: dict):
    ...
    is_estiva_product = product.get("site") == "estiva"

    for variant in variant_rows:
        color = clean_text(variant["color"])
        size = clean_text(variant["size"])
        if is_estiva_product and (not color or not size or is_estiva_placeholder_option(color) or is_estiva_placeholder_option(size)):
            continue
        processed_count += 1
        variant_images = variant.get("images") or product.get("images", [])
        variant_stock = variant.get("stock")
        if variant_stock is None:
            variant_stock = 10 if is_estiva_product else 20
        model_code = product["model_code"]

        if is_estiva_product:
            color_token = normalize_token(color)
            size_token = normalize_token(size)
            barcode = f"{model_code}-{color_token}-{size_token}"
            stock_code = f"STK-{model_code}-{color_token}-{size_token}"
            values = {
                "Barkod": barcode,
                "Model Kodu": model_code,
                "Marka": product["brand"],
                "Kategori": template_category,
                "Para Birimi": product.get("currency", "TRY"),
                "Ürün Adı": product["product_name"],
                "Ürün Açıklaması": product.get("generated_description") or product.get("page_description", ""),
                "Piyasa Satış Fiyatı (KDV Dahil)": product.get("market_price") or ((product["price"] or 0) + product.get("market_price_delta", 100)),
                "Trendyol'da Satılacak Fiyat (KDV Dahil)": product["price"],
                "Ürün Stok Adedi": product.get("stock_quantity", variant_stock),
                "Stok Kodu": stock_code,
                "KDV Oranı": product.get("vat_rate", 10),
                "Beden": size,
                "Renk": color,
                "Menşei": product.get("country_of_origin", "TR"),
                "Web Color": color,
                "Desi": product.get("desi", 1),
            }
            for shipping_header in ("Sevkiyat Süresi", "Teslimat Süresi"):
                if shipping_header in headers:
                    values[shipping_header] = product.get("shipping_time", 2)
        else:
...
def process_sources(args_dict: dict):
    output_path = args_dict.get("output") or build_default_output_path(args_dict["template"])
    ...
    overall = {
        ...
        "removed_invalid_estiva_rows": session.get("removed_invalid_estiva_rows", 0),
    }
    ...
    for idx, item in enumerate(source_items, start=1):
        ...
        try:
            ...
            if item["kind"] == "url" and product.get("site") != "estiva":
...
def main():
    ...
    print("Silinen eski hatalı Estiva satır sayısı:", result["removed_invalid_estiva_rows"])
    print("Not: Program açılışta aynı barkodlu eski tekrar satırları ve önceki hatalı Estiva varyant satırlarını temizler; sonra aynı barkod varsa satırı günceller, yeni barkodları sona ekler.")
